...
- [ ] **Backend markerer aldrig sessions som "completed"** — Alle sessions forbliver `status=active` selv når alle opgaver er løst. iOS workaround: `completedCount >= assignmentCount` bruges som completion-check. Backend bør sætte `status=completed` og `completed_at` når sidste assignment afsluttes. Opdaget 2026-04-12.
- [ ] **Duplikerede weekly sessions** — Der oprettes nye weekly sessions uden at rydde op i gamle. Én elev har 15 "Ugematematik — uge 15" sessions, de fleste med 0 opgaver løst. Backend bør enten genbruge eksisterende session eller rydde tomme op. Opdaget 2026-04-12.
